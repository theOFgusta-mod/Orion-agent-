    ...
    <PathFormatterProvider path={session()?.directory}>
      ...
      >
        <box flexDirection="row" flexGrow={1} minHeight={0}>
          <box flexGrow={1} minHeight={0} paddingBottom={1} paddingLeft={1} paddingRight={1} gap={1} border={RoundedBorder} borderColor={theme.border}>
            <Show when={session()}>
              <scrollbox
                ref={(r) => (scroll = r)}
                viewportOptions={{
...
function AssistantMessage(props: { message: AssistantMessage; parts: Part[]; last: boolean }) {
  ...
  const mood = createMemo(() => {
    ...
    if (!final()) return "🤔"
    const hasCode = props.parts.some((p) => p.type === "text" && p.text.includes("```"))
    if (hasCode) return "✨"
    return "😊"
  })
...
function TextPart(props: { last: boolean; part: TextPart; message: AssistantMessage }) {
  ...
  const mood = createMemo(() => {
    if (props.message.error) return "😅"
    if (!props.message.finish) return "🤔"
    return "😊"
  })
  const agentColor = createMemo(() => local.agent.color(props.message.agent))
  return (
    <Show when={props.part.text.trim()}>
      <box id={"text-" + props.part.id} marginTop={1} marginLeft={1} marginRight={4} flexShrink={0}>
        <box
          border={["all"]}
