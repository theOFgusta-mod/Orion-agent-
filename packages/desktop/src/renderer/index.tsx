...
render(() => {
  ...
  function Inner() {
    ...
    // Wallpaper: load from store and apply as background
    const applyWallpaper = async () => {
      try {
        const wallpaper = await window.api.getWallpaper()
        const root = document.getElementById("root")
        if (!root) return

        if (wallpaper) {
          root.style.backgroundImage = `url(${wallpaper.dataUrl})`
          ...
          const overlayColor = mode === "dark" ? "rgba(0,0,0,0.55)" : "rgba(255,255,255,0.25)"
          root.style.boxShadow = `inset 0 0 0 9999px ${overlayColor}`
        } else {
          root.style.backgroundImage = ""
          root.style.boxShadow = "none"
          root.style.backgroundBlendMode = "normal"
        }
      } catch {
    ...
    onMount(() => {
      void applyWallpaper()
      // Re-apply wallpaper when changed from settings
      const clear = makeEventListener(window, "wallpaper-changed", () => applyWallpaper())
      onCleanup(clear)
    })
