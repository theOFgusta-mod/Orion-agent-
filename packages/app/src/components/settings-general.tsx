  ...
  const AppearanceSection = () => (
    <div class="flex flex-col gap-1">
      ...
      <SettingsList>
        ...
        {/* Wallpaper — desktop only */}
        <Show when={desktop()}>
          {() => {
            const [wallpaperData, { refetch }] = createResource(() => window.api?.getWallpaper?.() ?? null)

            const pickWallpaper = async () => {
              const result = await window.api?.openFilePicker?.({
                title: language.t("settings.general.row.wallpaper.pickTitle"),
                extensions: ["png", "jpg", "jpeg", "webp", "bmp"],
              })
              if (!result || typeof result !== "string") return
              await window.api?.setWallpaperPath?.(result)
              refetch()
              window.dispatchEvent(new CustomEvent("wallpaper-changed"))
            }

            const removeWallpaper = async () => {
              await window.api?.setWallpaperPath?.(null)
              refetch()
              window.dispatchEvent(new CustomEvent("wallpaper-changed"))
            }
            ...
            return (
              <SettingsRow
                title={language.t("settings.general.row.wallpaper.title")}
                description={language.t("settings.general.row.wallpaper.description")}
              >
                <div class="flex items-center gap-2">
                  <Show when={wallpaperData()}>
                    {(data) => (
                      <div
                        class="w-10 h-10 rounded-md border border-border-weak-base bg-cover bg-center flex-shrink-0"
                        style={{
                          "background-image": `url(${data().dataUrl})`,
                  ...
                  <div class="flex gap-1">
                    <Button size="small" variant="secondary" onClick={pickWallpaper}>
                      <Icon name="photo" size="small" />
                      {language.t("settings.general.row.wallpaper.choose")}
                    </Button>
                    <Show when={wallpaperData()}>
                      <Button size="small" variant="secondary" onClick={removeWallpaper}>
